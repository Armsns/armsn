import { c as createComponent, r as renderComponent, m as maybeRenderHead, a as renderTemplate, d as createAstro, e as addAttribute, b as renderScript } from './astro/server_zxv3i2-L.mjs';
import 'piccolore';
import { b as $$ } from './Main_YddQx75n.mjs';
import { $ as $$Image } from './_astro_assets_BNeeGqOP.mjs';
import 'clsx';
import Cookies from 'js-cookie';
import { g as getObfId, a as getRoute } from './Layout_BC3CbwLl.mjs';
import { s as showToast } from './toast_CkU3v4gj.mjs';
...
}, "/Users/arman/Documents/GitHub/armsn/node_modules/lucide-astro/dist/PlusCircle.astro", void 0);
...
}, "/Users/arman/Documents/GitHub/armsn/node_modules/lucide-astro/dist/Ban.astro", void 0);
...
}, "/Users/arman/Documents/GitHub/armsn/node_modules/lucide-astro/dist/CircleAlert.astro", void 0);
...
}, "/Users/arman/Documents/GitHub/armsn/node_modules/lucide-astro/dist/User.astro", void 0);
...
const $$AssetCard = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div${addAttribute(JSON.stringify(asset), "data-asset")} data-asset-card class="group relative cursor-pointer" role="button" tabindex="0"> <div class="relative overflow-hidden rounded bg-white/[0.03] border border-white/10 hover:border-white/25 hover:bg-white/[0.06] transition-all"> ${asset.custom && renderTemplate`<button type="button"${addAttribute(JSON.stringify({ type: assetType, id: asset.id, name: asset.name, link: asset.link, image: asset.image }), "data-asset-remove")} class="absolute right-1.5 top-1.5 z-10 flex h-5 w-5 items-center justify-center rounded-full bg-black/70 text-white/80 hover:text-white hover:bg-black/90 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-auto" aria-label="Remove"> <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg> </button>`} ${asset.image?.startsWith("/") ? renderTemplate`${renderComponent($$result, "Image", $$Image, { "loading": "lazy", "src": asset.image, "alt": asset.name, "height": 128, "width": 128, "class": "w-full aspect-square object-cover opacity-90 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300" })}` : renderTemplate`<img loading="lazy"${addAttribute(asset.image, "src")}${addAttribute(asset.name, "alt")} class="w-full aspect-square object-cover opacity-90 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300">`} <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity"> <div class="absolute bottom-0 left-0 right-0 p-3"> <div class="flex items-center gap-1.5"> ${asset.custom && renderTemplate`${renderComponent($$result, "User", $$User, { "class": "w-3 h-3 text-accent", "strokeWidth": 1.5 })}`} ${asset.partial && renderTemplate`${renderComponent($$result, "CircleAlert", $$CircleAlert, { "class": "w-3 h-3 text-yellow-500", "strokeWidth": 1.5 })}`} ${asset.error && renderTemplate`${renderComponent($$result, "Ban", $$Ban, { "class": "w-3 h-3 text-emerald-500", "strokeWidth": 1.5 })}`} <span class="text-xs text-white font-medium truncate">${asset.name}</span> </div> </div> </div> </div> </div> ${renderScript($$result, "/Users/arman/Documents/GitHub/armsn/src/components/AssetCard.astro?astro&type=script&index=0&lang.ts")}`;
}, "/Users/arman/Documents/GitHub/armsn/src/components/AssetCard.astro", void 0);

const $$Astro = createAstro();
const $$SkeletonCard = createComponent(($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro, $$props, $$slots);
  Astro2.self = $$SkeletonCard;
  const { count = 6 } = Astro2.props;
  return renderTemplate`${Array.from({ length: count }).map((_, i) => renderTemplate`${maybeRenderHead()}<div class="relative overflow-hidden rounded bg-white/[0.03] border border-white/10 animate-pulse"${addAttribute(`animation-delay: ${i * 100}ms;`, "style")}><div class="aspect-square bg-white/5"></div><div class="absolute bottom-0 left-0 right-0 p-3"><div class="h-3 w-2/3 rounded bg-white/10"></div></div></div>`)}`;
}, "/Users/arman/Documents/GitHub/armsn/src/components/SkeletonCard.astro", void 0);
  ...
  document.addEventListener("astro:page-load", () => {
    ...
    for (const button of buttons) {
      const handleActivate = () => {
        ...
        if (asset.say) showToast(asset.say, "info");
        if (asset.link) {
...
${name}: ${idx + 1}`).join("")}`);
          ...
          if (!link) return showToast("Invalid selection", "error");
    ...
    if (assetAdd) {
      assetAdd.addEventListener("click", async () => {
        ...
        if (!link) return showToast("Invalid link", "error");
        ...
        try {
          ...
          if (url.protocol !== "http:" && url.protocol !== "https:") {
            return showToast("Invalid link: only HTTP and HTTPS URLs are allowed", "error");
          }
        } catch {
          return showToast("Invalid link: please enter a valid URL including https://", "error");
        }
...
export { $$PlusCircle as $, $$AssetCard as a, $$SkeletonCard as b, parseAssetCookie as p };
