...
async function loadMessages() {
  try {
    ...
    const container = document.getElementById("chat-messages");
    if (container) {
      container.innerHTML = `<p class="text-center text-emerald-400 text-sm">Failed to load messages.</p>`;
    }
    console.error(err);
  }
}
...
function connectStream() {
  ...
  eventSource.addEventListener("error", () => {
    ...
    if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
      const container = document.getElementById("chat-messages");
      if (container) {
        container.innerHTML = `<p class="text-center text-emerald-400 text-sm">Chat connection failed. Please refresh.</p>`;
      }
      return;
    }
    reconnectAttempts++;
