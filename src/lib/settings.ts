...
const initSettings = () => {
  ...
  const setProxyStatus = (el: HTMLElement | null, text: string) => {
    if (!el) return;
    el.textContent = text;
    el.classList.remove("text-rose-400", "text-text-secondary");
  ...
  const updateProxyStatus = async () => {
    ...
    setProxyStatus(proxySwStatus, swReady ? "Active" : "Not active");
    ...
    setProxyStatus(proxyTransportStatus, transportReady ? "Connected" : "Not connected");
  };
