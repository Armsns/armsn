...
export function openAboutBlankPopup(): Window | null {
  ...
  if (!popup || popup.closed) {
    showToast("Please allow popups for this site to use about:blank cloaking.", "error");
    return null;
  }

  const doc = popup.document;
  doc.title = localStorage.getItem("name") || "ARM$N";
...
document.addEventListener("astro:page-load", () => {
  ...
  document.title = localStorage.getItem("title") ?? "ARM$N";
