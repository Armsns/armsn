...
const IconButton = ({ onClick, icon: Icon, className = "", disabled = false, title = "" }: { onClick?: () => void; icon: ComponentType<{ className?: string }>; className?: string; disabled?: boolean; title?: string }) => (
  ...
  return (
    <div className="flex h-screen flex-col bg-background">
      <div className="flex items-center gap-1 bg-background-secondary/50 px-2 py-1.5 border-b border-border/50">
        {tabs.map((tab) => (
          <div key={tab.id} className={classNames(tabButtonClass, "group", tab.active ? "bg-background text-text border border-border/50" : "text-text-secondary hover:text-text hover:bg-white/5")}>
            <button type="button" onClick={() => setActiveTab(tab.id)} className="flex min-w-0 flex-1 items-center gap-2 text-left" aria-label={`Activate ${tab.title}`}>
              {favicons[tab.id] ? (
                <img
                  src={favicons[tab.id]}
                  alt=""
